...
class GridMaker(BaseGridMaker):
    ...
    def parse(self, **kwargs) -> List[Tuple[List[NDArray], List[NDArray], List[NDArray], List[NDArray], Dict]]:
        ...
        while num < num_samples:
            ...
            # random trajectory 추가 (칸수를 잘못 칠한 경로)
            for i in range(1):
    ...
    def random_trajectory(self, h, w, p_x, p_y, colors_list, choice):
        """칸수를 잘못 칠하는 random trajectory 생성
        - 순서는 정답과 동일 (choice=0: 위→아래, choice=1: 아래→위)
        - y_up/y_down의 증감(+1/-1)을 랜덤으로 선택
        """
        selections = []
        operations = []
