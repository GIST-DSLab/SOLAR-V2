"""1D-ARC category: 1d_denoising_1c — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    c=colors["c"]; w=random.randint(14,max_w); L=random.randint(4,min(w-6,12))
    s=random.randint(0,w-L); gi=np.zeros((1,w),int); gi[0,s:s+L]=c
    free=[x for x in range(w) if x<s-1 or x>s+L]  # noise away from block
    for x in random.sample(free,min(len(free),random.randint(1,4))): gi[0,x]=c
    go=np.zeros((1,w),int); go[0,s:s+L]=c
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O)
    b=_blocks(I[0]); big=max(b,key=lambda t:t[1]-t[0])
    noise=[(0,x) for (a,e,c) in b if (a,e,c)!=big for x in range(a,e+1)]
    return [0,SUBMIT],[sel_of(noise),_full(O)]
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_denoising_1c-maker_{_sn+1}","concept":"1D-ARC 1d_denoising_1c","operations":ops,"selections":sels}))
        return dataset
