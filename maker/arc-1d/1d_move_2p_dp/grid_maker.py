"""1D-ARC category: 1d_move_2p_dp — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    ...
    s=random.randint(0,3); dot=s+L+random.randint(2,6)
    if dot>=w: raise ValueError
    gi=np.zeros((1,w),int); gi[0,s:s+L]=c; gi[0,dot]=d
    go=np.zeros((1,w),int); go[0,s+2:s+L+2]=c; go[0,dot]=d
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O); b=_blocks(I[0]); blk=max(b,key=lambda t:t[1]-t[0])
    a,e,c=blk; cells=[(0,x) for x in range(a,e+1)]
    ops,sels=_move_ops(cells,MOVE_R,2); ops.append(SUBMIT); sels.append(_full(O)); return ops,sels
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_move_2p_dp-maker_{_sn+1}","concept":"1D-ARC 1d_move_2p_dp","operations":ops,"selections":sels}))
        return dataset
