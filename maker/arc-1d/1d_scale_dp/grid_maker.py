"""1D-ARC category: 1d_scale_dp — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    c,d=colors["c"],colors["d"]; w=random.randint(12,max_w); L=random.randint(4,min(w-6,14))
    s=0; gap=random.randint(2,5)
    if s+L+gap+1>=w: L=max(3,w-gap-2)
    dot=s+L+gap; gi=np.zeros((1,w),int); gi[0,s:s+L]=c; gi[0,dot]=d
    go=gi.copy(); go[0,s+L:dot]=c
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O); b=_blocks(I[0])
    blk=max(b,key=lambda t:t[1]-t[0]); a,e,c=blk
    dot=[t for t in b if t!=blk][0][0]
    cells=[(0,x) for x in range(e+1,dot)]
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_scale_dp-maker_{_sn+1}","concept":"1D-ARC 1d_scale_dp","operations":ops,"selections":sels}))
        return dataset
