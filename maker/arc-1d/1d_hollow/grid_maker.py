"""1D-ARC category: 1d_hollow — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    c=colors["c"]; w=random.randint(8,max_w); L=random.randint(3,min(w-1,12))
    s=random.randint(0,w-L); gi=np.zeros((1,w),int); gi[0,s:s+L]=c
    go=gi.copy(); go[0,s+1:s+L-1]=0
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O); (a,e,c)=_blocks(I[0])[0]
    cells=[(0,x) for x in range(a+1,e)]
    return [0,SUBMIT],[sel_of(cells),_full(O)]
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_hollow-maker_{_sn+1}","concept":"1D-ARC 1d_hollow","operations":ops,"selections":sels}))
        return dataset
