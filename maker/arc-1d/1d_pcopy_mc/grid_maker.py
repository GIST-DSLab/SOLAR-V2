"""1D-ARC category: 1d_pcopy_mc — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    ...
    while pos<w-L//2-1 and len(seeds)<3:
        sc=random.choice([k for k in range(1,10) if k!=c]); gi[0,pos]=sc; seeds.append((pos,sc)); pos+=random.randint(4,7)
    go=gi.copy()
    for p,sc in seeds: go[0,p-L//2:p-L//2+L]=sc
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O); b=_blocks(I[0]); L=b[0][1]-b[0][0]+1
    ops=[]; sels=[]
    for (a,e,c) in b[1:]:
        cells=[(0,x) for x in range(a-L//2,a-L//2+L)]; ops.append(c); sels.append(sel_of(cells))
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_pcopy_mc-maker_{_sn+1}","concept":"1D-ARC 1d_pcopy_mc","operations":ops,"selections":sels}))
        return dataset
