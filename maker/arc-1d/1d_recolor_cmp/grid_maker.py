"""1D-ARC category: 1d_recolor_cmp — auto-generated maker (bg=0)."""
...
def sample_colors():
    c=random.choice(range(1,10)); nc=random.choice([k for k in range(1,10) if k!=c]); return {"c":c,"nc":nc}
def generate(max_w,colors):
    c=colors["c"]; w=random.randint(16,max_w); gi=np.zeros((1,w),int); pos=0; lens=[]
    while pos<w-2 and len(lens)<5:
        if random.random()<0.6:
            L=random.randint(1,5)
            if pos+L>w: break
            gi[0,pos:pos+L]=c; lens.append((pos,L)); pos+=L+random.randint(2,3)
        else: pos+=random.randint(1,3)
    if not lens: gi[0,0:4]=c; lens=[(0,4)]
    go=gi.copy(); mx=max(L for _,L in lens)
    for (a,L) in lens:
        if L==mx: go[0,a:a+L]=colors["nc"]
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O); b=_blocks(I[0]); mx=max(e-a for a,e,c in b)
    cells=[(0,x) for (a,e,c) in b if e-a==mx for x in range(a,e+1)]
    nc=int(O[0,cells[0][1]])
    return [nc,SUBMIT],[sel_of(cells),_full(O)]
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_recolor_cmp-maker_{_sn+1}","concept":"1D-ARC 1d_recolor_cmp","operations":ops,"selections":sels}))
        return dataset
