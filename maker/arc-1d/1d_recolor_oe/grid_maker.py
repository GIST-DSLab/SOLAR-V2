"""1D-ARC category: 1d_recolor_oe — auto-generated maker (bg=0)."""
...
def sample_colors():
    co,ce=random.sample(range(1,10),2); return {"co":co,"ce":ce,"c":random.choice(range(1,10))}
def generate(max_w,colors):
    ...
    while pos<w-2:
        if random.random()<0.55:
            ...
            if pos+L>w: break
            gi[0,pos:pos+L]=c; pos+=L+random.randint(2,3); any_=True
        else: pos+=random.randint(1,3)
    if not any_: gi[0,0:3]=c
    go=gi.copy()
    for (a,e,col) in _blocks(gi[0]):
        go[0,a:e+1]=colors["co"] if (e-a+1)%2 else colors["ce"]
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O); odd=[]; even=[]
    for (a,e,col) in _blocks(I[0]):
        (odd if (e-a+1)%2 else even).extend([(0,x) for x in range(a,e+1)])
    ops=[]; sels=[]
    if odd: ops.append(int(O[0,odd[0][1]])); sels.append(sel_of(odd))
    if even: ops.append(int(O[0,even[0][1]])); sels.append(sel_of(even))
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_recolor_oe-maker_{_sn+1}","concept":"1D-ARC 1d_recolor_oe","operations":ops,"selections":sels}))
        return dataset
