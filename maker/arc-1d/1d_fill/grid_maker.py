"""1D-ARC category: 1d_fill — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    c=colors["c"]; w=random.randint(8,max_w)
    p1=random.randint(0,w-3); p2=random.randint(p1+2,w-1)
    gi=np.zeros((1,w),int); gi[0,p1]=c; gi[0,p2]=c
    go=gi.copy(); go[0,p1:p2+1]=c
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O)
    b=_blocks(I[0]); (a,_,c)=b[0]; (_,e,_)=b[-1]
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_fill-maker_{_sn+1}","concept":"1D-ARC 1d_fill","operations":ops,"selections":sels}))
        return dataset
