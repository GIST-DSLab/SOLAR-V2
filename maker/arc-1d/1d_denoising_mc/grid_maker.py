"""1D-ARC category: 1d_denoising_mc — auto-generated maker (bg=0)."""
...
def sample_colors():
    c=random.choice(range(1,10)); return {"c":c}
def generate(max_w,colors):
    c=colors["c"]; w=random.randint(14,max_w); L=random.randint(6,min(w-2,16))
    s=random.randint(0,w-L); gi=np.zeros((1,w),int); gi[0,s:s+L]=c
    others=[k for k in range(1,10) if k!=c]
    for x in random.sample(range(s,s+L),random.randint(1,max(1,L//4))): gi[0,x]=random.choice(others)
    go=np.zeros((1,w),int); go[0,s:s+L]=c
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); O=np.asarray(O)
    nz=np.where(O[0]!=0)[0]; c=int(O[0,nz[0]])
    bad=[(0,int(x)) for x in nz if I[0,x]!=c]
    return [c,SUBMIT],[sel_of(bad),_full(O)]
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_denoising_mc-maker_{_sn+1}","concept":"1D-ARC 1d_denoising_mc","operations":ops,"selections":sels}))
        return dataset
