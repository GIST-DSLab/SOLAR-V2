"""1D-ARC category: 1d_move_1p — auto-generated maker (bg=0)."""
...
def generate(max_w,colors):
    c=colors["c"]; w=random.randint(12,max_w); L=random.randint(4,min(w-4,16)); s=random.randint(0,w-L-3)
    gi=np.zeros((1,w),int); gi[0,s:s+L]=c; go=np.zeros((1,w),int); go[0,s+1:s+L+1]=c
    return {"input":gi,"output":go}
def derive_operations(I,O):
    I=np.asarray(I); nz=np.where(I[0]!=0)[0]; cells=[(0,int(x)) for x in nz]
    ops,sels=_move_ops(cells,MOVE_R,1); ops.append(SUBMIT); sels.append(_full(np.asarray(O))); return ops,sels
    ...
    def parse(self, **kwargs):
        ...
        for _sn in range(num_samples):
            ...
            dataset.append((ex_in,ex_out,pr_in,pr_out,
                {"id":f"1d_move_1p-maker_{_sn+1}","concept":"1D-ARC 1d_move_1p","operations":ops,"selections":sels}))
        return dataset
